...
static const char *OutputFilenameT = "llvmtimeprof.out";
 ...
 */
void write_profiling_data_d(double *Start,
   ...
   */
  if (OutFile == -1) {
    OutFile = open(OutputFilenameT, O_CREAT | O_WRONLY | O_APPEND, 0666);
    if (OutFile == -1) {
  ...
  write(OutFile, Start, NumElements*sizeof(double));
}
